...
function identify() {
    ...
    x = 0;
          ...
          // google.maps.event.trigger(map.data, 'click');
               map.events.register("click", map, function (event) {
                   alert(" (X):  " + event.xy.x + " (Y):  " + event.xy.y + "  event: " + selected_layers_name.join(','));
    ...
    //        $.ajax({
    //            data: {
    //                width: map.getSize().w, height: map.getSize().h, x: e.xy.x, y: e.xy.y,
    //                layers: selected_layers_name.join(',')
    //            },
    //            type: 'GET',
